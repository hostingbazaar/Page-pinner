document.addEventListener("DOMContentLoaded", function() {
  ...
  function fetchAndDisplayData() {
    ...
    fetch(firebaseUrl)
      ...
      .then(data => {
        currentData = Object.values(data).filter(item => item.userid === retrievedValue);
      
        let length = currentData.length;
        
      if(length===0){
        document.getElementById("image").innerHTML += "<img src='asset/images/nodata.png' class='nodata'><h3>No Data Found</h3>";
      }
      else{
        displayData(currentData);
        updatePagination(currentData);
